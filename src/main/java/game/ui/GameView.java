...
public class GameView {
    ...
    public GameView(GameState state, Runnable onRestart, Stage stage) {
        ...
        Font.loadFont(getClass().getResourceAsStream("/fonts/Faith Collapsing.ttf"), 16);
    ...
    private void renderMap(GraphicsContext gc, int tileSize, Font tileFont, int offsetY) {
        ...
        for (Enemy enemy : state.getEnemies()) {
            ...
            // Draw enemy sprite if available, otherwise fall back to text glyph
            String enemyPath = enemy.getSpritePath();
            if (enemyPath != null) {
                Image enemySprite = spriteCache.computeIfAbsent(enemyPath, p -> {
                    var stream = getClass().getResourceAsStream(p);
                    return stream != null ? new Image(stream) : wallSprite;
                });
                gc.drawImage(enemySprite, px, py, tileSize, tileSize);
            } else {
                gc.setFont(tileFont);
            ...
            // Thin HP bar above the enemy sprite / glyph
